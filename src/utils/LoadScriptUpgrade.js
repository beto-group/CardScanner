...
async function loadScript(dc, src, options = {}) {
  ...
  // Resolve custom cacheDir or default to CARD SCANNER local path
  const resolvedCacheDir = cacheDir ? dc.resolvePath(cacheDir) : dc.resolvePath("_RESOURCES/DATACORE/_DONE/CARD SCANNER/data/cache/scripts");
    ...
    try {
      ...
      if (isUrl) {
        ...
        if (scriptContent === null && cacheDir) {
          const defaultCacheDir = dc.resolvePath("_RESOURCES/DATACORE/_DONE/CARD SCANNER/data/cache/scripts");
...
async function fetchAndCacheImage(dc, url) {
  const cacheDir = dc.resolvePath("_RESOURCES/DATACORE/_DONE/CARD SCANNER/data/cache/images");
  const adapter = dc.app.vault.adapter;
